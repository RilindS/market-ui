...
const CashRecordsPage = () => {
  ...
  const formatDate = (dateString) => {
    ...
    const monthName = shqipMonths[date.getMonth()];
  ...
  /* =============================
     TOTALS (VETËM FAQJA AKTUALE)
  ============================= */
  const totals = records.reduce(
    (acc, r) => {
      acc.opening += r.openingBalance || 0;
      acc.sales += r.totalSales || 0;
      acc.purchases += r.totalPurchases || 0;
      acc.debt += r.debtPayments || 0;
      acc.closing += r.closingBalance || 0;
      return acc;
    },
    {
      opening: 0,
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#f0f0f0" }}>
              <th style={th}>Data</th>
              <th style={th}>Gjendja Fillestare</th>
              <th style={th}>Shitje</th>
              <th style={th}>Blerje</th>
              <th style={th}>Borgje të Paguara</th>
              <th style={th}>Gjendja Finale</th>
            </tr>
          </thead>
          ...
          <tbody>
            ...
            {records.length > 0 && (
              <tr style={{ fontWeight: "bold", background: "#e8f5e9" }}>
                <td style={td}>TOTAL (Faqja Aktuale)</td>
                <td style={td}>{totals.opening.toFixed(2)}</td>
                <td style={td}>{totals.sales.toFixed(2)}</td>
                <td style={td}>{totals.purchases.toFixed(2)}</td>
                <td style={td}>{totals.debt.toFixed(2)}</td>
                <td style={td}>{totals.closing.toFixed(2)}</td>
              </tr>
            )}
          </tbody>
        </table>
